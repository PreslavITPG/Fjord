            ...
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            if (MessageBox.Show("A tab with this name already exists. Overwrite?",
                        ...
                        using (var connection = new MySqlConnection(connectionString))
                        {
                            ...
                            string deleteQuery = "DELETE FROM tabs WHERE title = @title";
                            ...
                            // Then insert new
                            string insertQuery = "INSERT INTO tabs (title, filepath) VALUES (@title, @filepath)";
                            using (var insertCmd = new MySqlCommand(insertQuery, connection))
                            {
                                insertCmd.Parameters.AddWithValue("@title", title);
                                insertCmd.Parameters.AddWithValue("@filepath", targetPath);
                    ...
                    {
                        MessageBox.Show($"Error adding tab: {ex.Message}", "Error",
                            MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
